...
function contactPage(lang) {
  ...
  const l = labels[lang];
  const mapQuery = encodeURIComponent(config.addressEn);
  const mapTitle = lang === "ar" ? "موقع AES Egypt" : "AES Egypt Location";
  const mapText = lang === "ar"
    ? "اعرض العنوان على الخريطة قبل زيارتنا في المهندسين، الجيزة."
    : "View the address on the map before visiting our Mohandeseen, Giza office.";
  ...
  const body = `
    ...
    <section class="section">
      ...
      <div class="container map-card">
        <div class="map-copy">
          <p class="eyebrow">${esc(l.address)}</p>
          <h2>${esc(mapTitle)}</h2>
          <p>${esc(mapText)}</p>
          <a class="text-link" href="https://www.google.com/maps/search/?api=1&query=${mapQuery}" target="_blank" rel="noopener">${esc(mapLinkText)}</a>
        </div>
        <iframe title="${esc(mapTitle)}" src="https://www.google.com/maps?q=${mapQuery}&output=embed" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
      </div>
    </section>`;
